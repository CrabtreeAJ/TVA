...
class VotingSystemATVA1(VotingSystem):
    @override
    def simulate(self, strategy_type=None, cheating_probs=None, to_print: bool = True):
        """
        ATVA-1: Coalition voting allowed.
        No counter-strategic responses.
        Perfect knowledge.
        """

        true_prefs = self.true_preferences
        n = len(true_prefs)

        honest_winner = self.vote(true_prefs)[0][0]
        honest_happiness = [
            BasicHappiness(honest_winner, true_prefs)
            .get_happiness_single(true_prefs[i], honest_winner)
            for i in range(n)
        ]

        # Try coalitions of size >= 2
        for size in range(2, n + 1):
            for coalition in combinations(range(n), size):

                perm_lists = [
                    list(permutations(true_prefs[i]))
                    for i in coalition
                ]

                for perm_combo in product(*perm_lists):

                    situation = np.array(true_prefs.copy())

                    for idx, voter_id in enumerate(coalition):
                        situation[voter_id] = perm_combo[idx]

                    winner = self.vote(situation)[0][0]

                    improvements = []
                    for voter_id in coalition:
                        new_hap = BasicHappiness(winner, true_prefs) \
                            .get_happiness_single(true_prefs[voter_id], winner)
                        improvements.append(new_hap > honest_happiness[voter_id])

                    if all(improvements):
                        if to_print:
                            print(f"Coalition {coalition} manipulated successfully.")
                            print(f"New winner: {winner}")

                        return self.vote(situation)

        if to_print:
            print("No coalition manipulation found under ATVA-1.")

        return self.vote(true_prefs)

class VotingSystemATVA2(VotingSystem):

    def best_response(self, profile, voter_id):
        true_prefs = self.true_preferences
        true_ballot = true_prefs[voter_id]

        current_winner = self.vote(profile)[0][0]
        best_ballot = profile[voter_id]
        best_happiness = BasicHappiness(current_winner, true_prefs) \
            .get_happiness_single(true_ballot, current_winner)

        for perm in permutations(true_ballot):
            temp = profile.copy()
            temp[voter_id] = perm

            winner = self.vote(temp)[0][0]
            new_hap = BasicHappiness(winner, true_prefs) \
                .get_happiness_single(true_ballot, winner)

            if new_hap > best_happiness:
                best_happiness = new_hap
                best_ballot = perm

        return best_ballot

    def apply_counter_responses(self, profile, excluding_voter):
        for j in range(len(profile)):
            if j != excluding_voter:
                profile[j] = self.best_response(profile, j)
        return profile

    @override
    def simulate(self, strategy_type=None, cheating_probs=None, to_print: bool = True):
        """
        ATVA-2: Counter-strategic voting allowed.
        Single manipulator.
        Perfect knowledge.
        """
        ...
        honest_profile = self.apply_counter_responses(
            np.array(true_prefs.copy()), -1
        )

        honest_winner = self.vote(honest_profile)[0][0]
        ...
        for voter_id in range(n):
            ...
            best_strategy = None
            best_happiness = BasicHappiness(honest_winner, true_prefs) \
                .get_happiness_single(true_ballot, honest_winner)

            for perm in permutations(true_ballot):

                situation = np.array(true_prefs.copy())
                situation[voter_id] = perm

                situation = self.apply_counter_responses(situation, voter_id)
            ...
            if best_strategy is not None:
                if to_print:
                    print(f"Voter {voter_id} manipulated under ATVA-2.")
                situation = np.array(true_prefs.copy())
                situation[voter_id] = best_strategy
                situation = self.apply_counter_responses(situation, voter_id)
                return self.vote(situation)

        if to_print:
            print("No profitable manipulation under ATVA-2.")
